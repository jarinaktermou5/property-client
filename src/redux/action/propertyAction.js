...
export const getAllProperty = (category, price = [5000, 5000], keyword) => async (dispatch) => {
    try {
        ...
        let link = `https://rental-property-server.vercel.app/api/v1/properties?keyword=${keyword}&price[gte]=${price[0]}&price[lte]=${price[1]}`;
        if (category) {
            link = `https://rental-property-server.vercel.app/api/v1/properties?keyword=${keyword}&price[gte]=${price[0]}&price[lte]=${price[1]}&category=${category}`;
        }
...
export const createProperty = (propertyData) => async (dispatch) => {
    try {
        ...
        const data = await axios.post('https://rental-property-server.vercel.app/api/v1/property/new', propertyData);
        dispatch({
...
export const getPropertyDetails = (id) => async (dispatch) => {
    ...
    try {
        ...
        const { data } = await axios.get(`https://rental-property-server.vercel.app/api/v1/property/${id}`, config);
        dispatch({
